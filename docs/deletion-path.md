...
此去重仅适用于新固定标识，不重写或自动合并旧版随机 ID 的逐项记录。最终汇总自动补记与面向普通用户的历史展示仍待处理；不得跨汇总和逐项记录直接累加文件数。本地逐项记录含原始路径，不可作为脱敏报告上传。当前每次去重顺序读取完整日志，大批量还原的 IO 成本及超长历史行的内存边界尚待基准与优化，不声称满足性能目标。
