...
每项移动或恢复核对成功后，先追加并刷新 `restore-file` 历史，再移除对应恢复意图。逐项记录包含批次 ID、原路径、隔离路径、固定还原目标、内容摘要，以及是否为重试核对。审计失败立即停止后续移动，保留 restoring 意图；日志恢复可写后可重试核对并补记。最终 `restore` 汇总失败仍抛出异常，但已移除的条目有逐项证据，不再仅留下批次起始记录。

逐项审计目前为至少一次记录：若审计成功后清单更新失败，重试可能追加同一条目的核对记录，不得直接把这些记录的 FileCount 相加当作唯一文件数。最终汇总自动补记、记录去重及面向普通用户的历史展示仍待处理；本地逐项记录含原始路径，不可作为脱敏报告上传。
