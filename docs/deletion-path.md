...
持锁后先补记已有还原收尾凭据；补记失败不开始新的写操作。该锁只协调使用这一协议的 Kleaner 操作；不替代 reparse point 校验，也未证明对外部恶意路径替换、不同根目录嵌套/别名及所有文件系统语义均有隔离效果。上述风险与其他审计中断窗口仍属工单 10 未完成项。
...
在整批路径校验后、起始审计及首次移动前，在根目录 `.pending-audit/<GUID>.json` 原子初始化凭据（版本、唯一 ID、批次 ID、初始计数 0、尚未确认的结果）。初始化失败拒绝开始还原。完成逐项循环后、删除空批次前，更新本次恢复数；该更新失败就不删除批次清单。收尾后原子保存实际 ok/partial，再用固定 `restore-summary:<GUID>` 追加历史，成功刷新后才移除凭据。

`RecoverPendingAudit()` 只补记历史、不还原或清空文件；所有写操作获得根锁后也先执行补记。若进程在最终结果落盘前退出，下次只记 partial 并注明最终状态未确认、数量仅为已持久化下限，不推断成功：例如首项已移动但尚未更新计数时，初始 0 不能解读为“没有文件移动”。准确文件状态仍需通过批次还原意图与内容核对。已有最终结果则原样补记。历史已写而凭据尚存时，固定 ID 防止重复追加。凭据限制 65536 字节，校验版本、ID 与文件名、批次路径、非负计数及结果；损坏凭据或目录被文件占用时拒绝新操作，不覆盖证据。遗留 `.tmp` 文件保留，不当作已提交凭据。

五个独立进程退出点（起始后、首项移动但未记逐项历史、收尾准备后、结果落盘后、历史追加后）和历史被锁定后的补记已验收。早期中断先补记未确认事件，再显式还原可恢复夹具文件。初始化目录冲突不移动文件，计数凭据替换被锁定时保留批次清单。清理/永久清空的汇总恢复、中断事件的精确计数核对、凭据来源认证、外部路径竞争和 UI 待处理提示仍未完整解决。不能据此宣称所有审计事务已经闭环。
