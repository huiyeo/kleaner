...
完成逐项循环后、删除空批次前，在根目录 `.pending-audit/<GUID>.json` 原子保存收尾凭据（版本、唯一 ID、批次 ID、本次恢复数、尚未确认的结果）。凭据刷新失败就不删除批次清单。收尾后原子保存实际 ok/partial，再用固定 `restore-summary:<GUID>` 追加历史，成功刷新后才移除凭据。

`RecoverPendingAudit()` 只补记历史、不还原或清空文件；所有写操作获得根锁后也先执行补记。若进程在收尾结果落盘前退出，下次只记 partial 并注明最终状态未确认，不推断成功；已有最终结果则原样补记。历史已写而凭据尚存时，固定 ID 防止重复追加。凭据限制 65536 字节，校验版本、ID 与文件名、批次路径、非负计数及结果；损坏凭据或目录被文件占用时拒绝新操作，不覆盖证据。遗留 `.tmp` 文件保留，不当作已提交凭据。

三个独立进程退出点（准备后、结果落盘后、历史追加后）和历史被锁定后的补记已验收。此协议仅覆盖还原收尾：逐项循环到首次凭据写入之间的中断、更早阶段中断、清理/永久清空的汇总恢复、凭据来源认证、外部路径竞争和 UI 待处理提示仍未完整解决。不能据此宣称所有审计事务已经闭环。
