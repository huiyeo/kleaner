...
| 路径 | 步骤 | 预期结果 | 已验证 |
|---|---|---|---|
| 全新安装 | 运行 Setup.exe → 完成安装 → 启动 | 桌面/开始菜单入口可用；应用正常扫描；`%APPDATA%\Kleaner\` 生成 | v0.1.0 本机实测 |
| 覆盖升级 | 在旧版本上运行新版 Setup.exe | 应用可启动；`settings.json` 与 `history.jsonl` 保留且可读；历史窗口旧记录显示「未链段」提示不报断裂 | 待执行 |
| 升级中断 | 升级过程中强制结束安装器 | 旧版本仍可启动；再次运行新版 Setup 可完成升级 | 待执行 |
| 回退 | 重新安装旧版 Setup.exe | 应用可启动；新版写入的历史记录仍可展示（链向后兼容）；隔离区批次仍可还原 | 待执行 |
| 卸载 | 系统设置 → 应用 → 卸载 | 应用移除；隔离区目录与 `%APPDATA%\Kleaner\` 的处置（保留/提示删除）与发布说明一致 | 待执行 |
