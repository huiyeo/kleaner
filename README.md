...
质量：`dotnet test Kleaner.slnx -c Release` 当前 105/105 通过，覆盖规则校验、扫描/年龄阈值/keepNewest 语义、重复文件选择策略、清理计划入口与执行前复验、部分清空后的清单保留、路径校验、持久化故障及独立进程中断后的文件恢复。中断遗留临时清单会保留并报告 partial；自动收尾、最终审计失败、断电及并发路径变化仍待验证，测试数不代表 Phase 0 已达标。
