...
质量：`dotnet test Kleaner.slnx -c Release` 当前 160/160 通过，覆盖规则校验、扫描/年龄阈值/keepNewest 语义、重复文件选择策略、清理计划入口与执行前复验、部分清空后的清单保留、路径校验、持久化故障，以及清理/还原/清空汇总审计在独立进程中断后的补记。中断遗留临时清单会保留并报告 partial；移动路径的检查-移动间隙由句柄锚定关闭（ADR 0003）；凭据来源认证在同权限攻击者模型下不可达成，按 ADR 0002 明确为已知边界。断电场景与历史防篡改（工单 14）仍待实现，测试数不代表 Phase 0 已达标。
