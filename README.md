...
质量：`dotnet test Kleaner.slnx -c Release` 当前 152/152 通过，覆盖规则校验、扫描/年龄阈值/keepNewest 语义、重复文件选择策略、清理计划入口与执行前复验、部分清空后的清单保留、路径校验、持久化故障，以及清理/还原/清空汇总审计在独立进程中断后的补记。中断遗留临时清单会保留并报告 partial；断电、凭据来源认证与外部路径竞争仍待验证，测试数不代表 Phase 0 已达标。
